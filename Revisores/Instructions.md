...
Por cada revisor, en el arreglo `REVISORES` dentro de `script.js`:
...
- `script.js`: un solo archivo con datos + lógica, igual patrón que
  `Catalogo_Articulos/script.js`. Primero declara `var REVISORES = [...]`
  (334 registros) y, justo después, el IIFE con toda la lógica (búsqueda,
  paginación, render, `COUNTRY_NAMES`). Se carga con un único
  `<script src="script.js">` en `index.html`. Antes estaba dividido en
  `revisores-data.js` + `script.js`; se fusionó porque cargarlos como dos
  `<script src>` separados no garantizaba el orden de ejecución en WP
  Bakery y a veces el IIFE corría antes de que `REVISORES` existiera
  (`ReferenceError: REVISORES is not defined`).
...
1. **`script.js`:** edita el arreglo `REVISORES` directamente, al
   principio del archivo, antes del IIFE (mismo formato: `id`, `nombre`,
   `apellido`, `afiliacion`, `pais`, `codigo`).
...
`script.js` declara el arreglo `REVISORES` como variable global al inicio
del archivo (no se hace `fetch()` de ningún archivo), así que `index.html`
se puede abrir directamente con doble clic (protocolo `file://`) sin
